 ...
 */
const cancelProductRequestService = async (productRequest, params) => {
  try {
    const { cancelledBy, auditContext } = params;

    if (productRequest.status !== RequestStatus.PENDING) {
      logWithTime(`❌ [cancelProductRequestService] Cannot cancel non-pending request`);
      return {
        errorCode: BAD_REQUEST,
        isSuccess: false,
        description: "Only product requests in PENDING status can be cancelled"
      };
    }
    ...
    const description = `Product request '${productRequest.title}' (${productRequest._id}) cancelled by ${cancelledBy}.`;
    ...
    logWithTime(`❌ [cancelProductRequestService] Error: ${error.message}`);
    ...
    return {
      ...
      isSuccess: false,
      description: "Internal error while cancelling product request"
    };
  }
};

module.exports = { cancelProductRequestService };
